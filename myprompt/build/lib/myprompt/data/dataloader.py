...
# class NLIDataLoader(PromptDataLoader):
#     InputFeatures.add_tensorable_keys('sample_id')
#     InputFeatures.add_keys('sample_id')
#     InputFeatures.add_tensorable_keys('true_label')
#     InputFeatures.add_keys('true_label')    
#     def __init__(self, 
#                  dataset: Union[Dataset, List],
#                  template: Template,
#                  tokenizer: PreTrainedTokenizer,
...
#         """
#         for idx, wrapped_example in tqdm(enumerate(self.wrapped_dataset), desc ='tokenizing'):
#             wrapped_example[1].update({'sample_id': self.raw_dataset[idx].meta['sample_id']})
#             wrapped_example[1].update({'true_label': self.raw_dataset[idx].meta['true_label']})
#             inputfeatures = InputFeatures(**self.tokenizer_wrapper.tokenize_one_example(wrapped_example, self.teacher_forcing), **wrapped_example[1]).to_tensor()
#             self.tensor_dataset.append(inputfeatures)


class PtuningDataLoader(object):
    def __init__(self, 
                 dataset: Union[Dataset, List],
                 tokenizer: PreTrainedTokenizer,
                 tokenizer_wrapper_class: TokenizerWrapper,
    ...
    def wrap_one_example(self,example):
        text = example.text_a
        ret1 = {'text':text, 'loss_ids':0, 'shortenable_ids':1}
        ret2 = {'guid':example.guid, 'label':example.label}
        return [[ret1],ret2]
    ...
    def wrap(self):
        ...
        if isinstance(self.raw_dataset, Dataset) or isinstance(self.raw_dataset, List):
            ...
            for idx, example in enumerate(self.raw_dataset):
                #wrapped_example = self.template.wrap_one_example(example)
                wrapped_example = self.wrap_one_example(example)
                self.wrapped_dataset.append(wrapped_example)
        else:
            raise NotImplementedError
