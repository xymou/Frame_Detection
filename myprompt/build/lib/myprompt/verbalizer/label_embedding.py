...
"""
用token avg 表示label embedding
考虑path 依赖; 前面加上父类/issue的label 信息
"""
...
class LabelEmbedVerbalizer(nn.Module):
        ...
        with torch.no_grad():
            ...
            for j in range(len(label_words)):
                ...
                for i, idx in enumerate(label_words[j]):
                    res.append(torch.mean(word_embeddings.weight[idx], dim=0).view(1,-1)) #每个token 取平均值得到这个短语的表示
                embeddings.append(torch.mean(torch.cat(res,dim =0), dim=0).view(1,-1)) #每个短语取平均值得到这个label word的表示     
    ...
    def process_logits(self, mlm_logits: torch.Tensor, **kwargs):
        # logits 是MLM的表示输出（不是过了30522全连接层后的）
